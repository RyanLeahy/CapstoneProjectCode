...
esp_err_t bno055_read_register(i2c_number_t i2c_num, bno055_reg_t reg, uint8_t *p_reg_val){
    ...
    if( !x_bno_dev[i2c_num].bno_is_open) {
        ESP_LOGE(BNO055_TAG, "bno055_read_register(): device is not open");
    ...
    switch (err) {
        ...
        case  ESP_ERR_TIMEOUT:
            ESP_LOGE(BNO055_TAG, "bno055_read_register(): i2c timeout");
            break;
        default: 
            ESP_LOGE(BNO055_TAG, "bno055_read_register(): failed");
    }
...
// --------|-----------------------------------|-------------------------|-----------------------|------|
 ...
 esp_err_t bno055_write_register(i2c_number_t i2c_num, bno055_reg_t reg, uint8_t reg_val){
    ...
    if( !x_bno_dev[i2c_num].bno_is_open) {
        ESP_LOGE(BNO055_TAG, "bno055_write_register(): device is not open");
    ...
    switch (err) {
        case ESP_OK: 
            break;
        case  ESP_ERR_TIMEOUT:
            ESP_LOGE(BNO055_TAG, "bno055_write_register(): i2c timeout");
            break;
        default: 
            ESP_LOGE(BNO055_TAG, "bno055_write_register(): failed");
    }
...
esp_err_t bno055_read_data(i2c_number_t i2c_num, bno055_reg_t start_reg, uint8_t *buffer, uint8_t n_bytes){
    ...
    if( !x_bno_dev[i2c_num].bno_is_open) {
        ESP_LOGE(BNO055_TAG, "bno055_read_data(): device is not open");
        return BNO_ERR_NOT_OPEN;
    }

    if( n_bytes < 2 || n_bytes > 0x7F ) {
        ESP_LOGE(BNO055_TAG, "bno055_read_data(): invalid number of bytes: %d", n_bytes);
    ...
    switch (err) {
        case ESP_OK: 
        	break;
        case ESP_ERR_TIMEOUT:
            ESP_LOGE(BNO055_TAG, "bno055_read_data(): i2c timeout");
            break;
        default: 
            ESP_LOGE(BNO055_TAG, "bno055_read_data(): failed");
    }
...
esp_err_t bno055_set_default_conf(bno055_config_t * p_bno_conf){
    ...
    p_bno_conf->sda_io_num = 8;        // GPIO number for I2C sda signal 25
    p_bno_conf->sda_pullup_en = GPIO_PULLUP_ENABLE;  // Internal GPIO pull mode for I2C sda signal
    p_bno_conf->scl_io_num = 9;        // GPIO number for I2C scl signal 26 
    ...
    p_bno_conf->timeout = 10*1000*80;     // 10ms in 80 MHz ticks, should be < 0xFFFFF
...
esp_err_t bno055_open(i2c_number_t i2c_num, bno055_config_t * p_bno_conf )
{
    ...
    if(x_bno_dev[i2c_num].bno_is_open) {
        ESP_LOGW(BNO055_TAG, "bno055_open(): device is already open");
    ...
    ESP_LOGD(BNO055_TAG, "i2c_param_config() returned %s", esp_err_to_name(err));
    if( err != ESP_OK ) return err;
        
    err = i2c_driver_install(i2c_num, I2C_MODE_MASTER,
                              I2C_MASTER_RX_BUF_DISABLE,
                              I2C_MASTER_TX_BUF_DISABLE, 0);
    ESP_LOGD(BNO055_TAG, "i2c_driver_install() returned %s", esp_err_to_name(err));
    if( err != ESP_OK ) return err;
    
    /*//CAUSES CHIP TO ERROR OUT
    err = i2c_set_timeout(i2c_num, p_bno_conf->timeout);
    ESP_LOGD(BNO055_TAG, "i2c_set_timeout() returned %s", esp_err_to_name(err));
    if( err != ESP_OK ) return err;*/
    ...
    uint8_t reg_val;
    vTaskDelay(850/ portTICK_PERIOD_MS); //Initial bootup can take 850ms apparently
    err = bno055_read_register(i2c_num, BNO055_CHIP_ID_ADDR, & reg_val);
    
    if( err == ESP_OK ) {
        
        ESP_LOGD(BNO055_TAG, "BNO055 ID returned 0x%02X", reg_val);
        if( reg_val == BNO055_ID ) {
            ESP_LOGI(BNO055_TAG, "BNO055 detected \n");
        } 
        else {
            ESP_LOGE(BNO055_TAG, "bno055_open() error: BNO055 NOT detected");
            goto errExit;
        }
    }
    ...
    ESP_LOGD(BNO055_TAG, "Set config oper. mode - Ok");

    // Reset
    err=bno055_write_register(i2c_num, BNO055_SYS_TRIGGER_ADDR, 0x20 );
    if(err != ESP_OK) goto errExit;
    vTaskDelay(700 / portTICK_PERIOD_MS);
    ESP_LOGD(BNO055_TAG, "BNO055 reset - Ok");
    ...
    ESP_LOGD(BNO055_TAG, "Set external oscillator - Ok");
    ...
    if(err != ESP_OK) goto errExit;
    vTaskDelay(20 / portTICK_PERIOD_MS);
    ESP_LOGD(BNO055_TAG, "Set normal power mode - Ok");
...
esp_err_t bno055_set_ext_crystal_use(i2c_number_t i2c_num, bool use_ext ){
    ...
    if( mode  != OPERATION_MODE_CONFIG ) {
        ESP_LOGE(BNO055_TAG, "bno055_set_ext_crystal_use(): device should be in the config mode. Current mode: %d", mode);
...
esp_err_t _bno055_buf_to_euler(uint8_t* buffer, bno055_vec3_t* euler)
{
    int16_t x, y, z;
    // combine MSB and LSB into 16-bit int
    z = (((uint16_t)buffer[1]) << 8) | ((uint16_t)buffer[0]); //heading - z
    y = (((uint16_t)buffer[3]) << 8) | ((uint16_t)buffer[2]); //roll - y
    x = (((uint16_t)buffer[5]) << 8) | ((uint16_t)buffer[4]); //pitch - x

    //1 degree = 16 LSB
    euler->x = ((double)x) / 16.0;
    euler->y = ((double)y) / 16.0;
    euler->z = ((double)z) / 16.0;

    return ESP_OK;
}

esp_err_t bno055_get_euler(i2c_number_t i2c_num, bno055_vec3_t* euler)
{
 ...
 * @name BNO055 IMU
 * 
 * @brief This handles the initialization of the BNO055 IMU. There will be no event handling for the IMU because the I2C bus only sends data after being asked to.
 * 
 * Default initialization:
 * Address: 0x28 (Address A)
 * SDA: GPIO 19
 * SCL: GPIO 20
 * PULLUP: Disabled
 * CLK: 400 kHz
 * timeout: 10ms
 * ext oscillator: false
 * 
 * @param i2c_num Holds the I2C number that the ESP will use for communicating with the BNO055
 * 
 * @return esp_err_t
 * 
 * @author Ryan Leahy
 * @date 02/21/2023
 * 
 * @cite https://www.bosch-sensortec.com/media/boschsensortec/downloads/datasheets/bst-bno055-ds000.pdf
*/
esp_err_t BNO055_init(i2c_number_t *i2c_num)
{
    ESP_LOGI(BNO055_TAG, "\n\n\n"
        "*******************\n"
    ...
    *i2c_num = I2C_NUMBER_0;
    
    esp_log_level_set(BNO055_TAG, ESP_LOG_DEBUG);

    esp_err_t err;
    err = bno055_set_default_conf(&bno_conf); 
    ESP_LOGI(BNO055_TAG, "bno055_set_default_conf() returned %s \n", esp_err_to_name(err));
    
    err = bno055_open(*i2c_num, &bno_conf);
    ESP_LOGI(BNO055_TAG, "bno055_open() returned %s \n", esp_err_to_name(err));
    
    if(err != ESP_OK) 
    {
        ESP_LOGW(BNO055_TAG, "Program terminated!\n");
        return err;
    }
    vTaskDelay(1000 / portTICK_PERIOD_MS);

    /**
     * Changes operation mode from CONFIGMODE to NDOF
     * 
     * NDOF mode enables the following:
     * Accelerometer
     * Magnetometer
     * Gyroscope
     * 
     * and has the BNO055 combine the data to give us:
     * Absolute orientation
    */
    err = bno055_set_opmode(*i2c_num, OPERATION_MODE_NDOF);
    ESP_LOGI(BNO055_TAG, "bno055_set_opmode(OPERATION_MODE_NDOF) returned %s \n", esp_err_to_name(err));
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    ...
    ESP_LOGI(BNO055_TAG, "System status: 0x%02X \n", system_status);
    ...
    ESP_LOGI(BNO055_TAG, "Self test result: 0x%02X \n", self_test_result);
    ...
    ESP_LOGI(BNO055_TAG, "System error: 0x%02X \n", system_error);
